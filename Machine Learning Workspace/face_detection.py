# this project is attributed to a separate workspace called 'untitled'. from now on make sure all projects are
...
we are using the PIL library to resize the image to our desired dimension
'''
# for i in range(630):
#     try:
...
if os.path.isfile(("C:/workstation/PycharmProjects/Machine Learning workspace/models/cnn.hfile")):
    ...
    if str(input()).strip().lower() == 'y':
        ...
        model.add(Conv2D(64, (3, 3), activation="relu"))
        model.add(MaxPooling2D((2, 2))) # we are reducing the matrix size.

        model.add(Flatten())
        model.add(Dropout(0.5)) # we remove certain unnecessary parts of the images.
        model.add(Dense(32, activation="relu")) # it converges the broken parts of the images.
...
# classifier
            faces = classifier.detectMultiScale(gray_frame, scaleFactor=1.3, minNeighbors=5)
            if str(type(faces)) == str(type(())):  # if the face is detected then display on the screen.
            ...
            for x, y, w, h in faces:
                crop_frame = gray_frame[y:y + h, x:x + w]  # we are only considering our Region of Interest.(ROI)
                r_img = cv2.resize(crop_frame, (100, 100))
    # m
                prediction = m.predict(r_img[np.newaxis, :, :, np.newaxis])  # np.newaxis adds a dimension to the array. 1d array becomes 2d arrays and so on, either rowwise or columnwise.
                name = names[np.argmax(prediction)]
                print(name)
                cv2.putText(img, name, (x, y),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)  # last 3 params: fontsize, color, stroke
