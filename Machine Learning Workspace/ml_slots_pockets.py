...
def edge_detection(img):
    # path = 'D:\\ML_datasets\\km_slot_pocket\\images\\valid\\pockets\\p_21.bmp'
    # img = cv2.imread(path)
    # img = cv2.blur(img, (3, 3))  # k = 1 / width * height therefore, (width, height)
    canny_img = cv2.Canny(img, 100, 200)
    coords = np.argwhere(canny_img > 0)
    x1, y1, x2, y2 = coords[:, 0].min(), coords[:, 1].min(), coords[:, 0].max(), coords[:, 1].max()
    img = img[y1:y2, x1:x2]  # syntax: (col, row)  img_shape(200, 150)
    return img


def resize_image(path, path_2):
    file_list = os.listdir("D:\\ML_datasets\\km_slot_pocket\\train\\pockets")
    save_loc = "D:\\ML_datasets\\km_slot_pocket\\images\\valid"
    # resize train images
    try:
...
def neural_net():
    ...
    path = 'D:\\ML_datasets\\km_slot_pocket\\images\\valid\\pockets\\p_54.bmp'
    # 'D:\\ML_datasets\\km_slot_pocket\\images\\valid\\slots\\s_2.bmp'
    ...
    cv2.imshow('GUI', display_img)
    cv2.waitKey()


# resize images.
r_path = "D:\\ML_datasets\\km_slot_pocket\\train"
r_path_2 = "D:\\ML_datasets\\km_slot_pocket\\test"
# resize_image(r_path, r_path_2)
